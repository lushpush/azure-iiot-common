...
namespace Microsoft.Azure.CosmosDB.BulkExecutor.Graph {
    ...
    public static class GremlinElementEx {
        ...
        /// <returns></returns>
        public static GremlinEdge ToEdge<V1, E, V2>(this E edge, V1 outV, V2 inV) {
            outV.ToJElement(out var outId, out var outLabel, out var outPk);
            inV.ToJElement(out var inId, out var inLabel, out var inPk);

            var jEdge = edge.ToJElement(out var edgeId, out var edgeLabel,
                out var edgePk);
        ...
        /// <returns></returns>
        public static GremlinVertex ToVertex<V>(this V vertex) {
            var jvertex = vertex.ToJElement(out var id, out var label, out var pk);
        ...
        /// <param name="pk"></param>
        /// <returns></returns>
        private static JObject ToJElement<T>(this T element, out string id,
            out string label, out string pk) {
            var jelement = JObject.FromObject(element);
